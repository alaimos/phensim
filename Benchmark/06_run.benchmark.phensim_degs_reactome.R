...
run.phensim <- function(id, sim.gene, sim.dir, non.exp) {
    MITHRIL.COMMAND     <- "C:\\Java\\bin\\java -jar %s/MITHrIL2.jar phensim -verbose -i %s -e mirna -enrichment-evidence-type STRONG -non-expressed-file %s -number-of-iterations 10 -t 14 -seed 1234 -reactome -fast -o %s"
    phensim.output.file <- file.path(getwd(), "phensim_degs_reactome", paste0("simulation_",id,".txt"))
...
export.subgraph <- function(id, sim.gene) {
    MITHRIL.COMMAND     <- "C:\\Java\\bin\\java -jar %s/MITHrIL2.jar export-subgraph -verbose -i %s -e mirna -enrichment-evidence-type STRONG -reactome -o %s"
    phensim.output.file <- file.path(getwd(), "phensim_degs_reactome", paste0("subgraph_",id,".txt"))
    if (!file.exists(phensim.output.file)) {
...
for (i in 1:nrow(datasets)) {
    ...
    phensim.output <- run.phensim(id, names(ds$degs.reactome), unname(ds$degs.reactome), non.exp)
    if (is.null(phensim.output)) next();
    subgraph.genes    <- export.subgraph(id, names(ds$degs.reactome))
...
saveRDS(mtx.results, file = "phensim_degs_reactome.rds")
