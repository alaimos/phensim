...
run.phensim <- function(id, sim.gene, sim.dir, non.exp) {
    ...
    phensim.output.file <- file.path(getwd(), "phensim_reactome", paste0("simulation_",id,".txt"))
...
export.subgraph <- function(id, sim.gene) {
    ...
    phensim.output.file <- file.path(getwd(), "phensim_reactome", paste0("subgraph_",id,".txt"))
    if (!file.exists(phensim.output.file)) {
...
for (i in 1:nrow(datasets)) {
    ...
    phensim.output <- run.phensim(id, sim.gene, sim.dir, non.exp)
    if (is.null(phensim.output)) next();
    subgraph.genes    <- export.subgraph(id, sim.gene)
