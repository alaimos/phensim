make.graph <- function (phensim, bionsi) {
    ...
    p0 <- ggplot(data = merged, aes(x=PHENSIM.Accuracy, y=BIONSI.Accuracy)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Overall Accuracy")
    
    p1 <- ggplot(data = merged, aes(x=PHENSIM.Predicted.PPV, y=BIONSI.Predicted.PPV)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Expressed Genes - PPV")
    
    p2 <- ggplot(data = merged, aes(x=PHENSIM.Predicted.SENS, y=BIONSI.Predicted.SENS)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Expressed Genes - Sensitivity")
    
    p3 <- ggplot(data = merged, aes(x=PHENSIM.Predicted.SPEC, y=BIONSI.Predicted.SPEC)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Expressed Genes - Specificity")
    
    p4 <- ggplot(data = merged, aes(x=PHENSIM.NonPredicted.PPV, y=BIONSI.NonPredicted.PPV)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Non-expressed Genes - PPV")
    
    p5 <- ggplot(data = merged, aes(x=PHENSIM.NonPredicted.FNR, y=BIONSI.NonPredicted.FNR)) + geom_point(aes(color=Dataset)) + 
        xlab("PHENSIM") + ylab("BioNSI") + ylim(c(0,1)) + xlim(c(0,1)) + geom_segment(aes(x=0, y=0, xend=1, yend=1), linetype=2) + ggtitle("Non-expressed Genes - FNR")
