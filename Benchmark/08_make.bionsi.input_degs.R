...
for (i in 1:nrow(datasets)) {
    ...
    tmp[tmp != norm] <- norm
    tmp[names(ds$degs)] <- unname(sapply(unname(ds$degs), function (j) (ifelse(j == "OVEREXPRESSION", over, undr))))
    ...
    write.table(df.tmp, file = file.path(getwd(), "bionsi_degs", paste0("exp",id,".csv")), sep = ",", quote = FALSE, row.names = FALSE, col.names = TRUE)
}
